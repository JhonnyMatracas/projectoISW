...
class Feedback extends Session
{
    ...
    {
        parent::addVector('input_message',$a,$b); // TODO: Change the autogenerated stub

    }

    public static function getInputMessage($a)
    {
        if (! is_null(Session::getOne('input_message',$a)) && count(Session::getOne('input_message',$a)) > 0) {
         $message = '<div><ul class="feedback_input_form">';
            //d(Session::getOne('input_message',$a));
            foreach(Session::getOne('input_message',$a) as $error){
                $message .= '<li>'. $error .'</li>';
            }
            $message .= '</ul></div>';
